...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(dense_folder, 'sparse', '0')
    cam_dir = os.path.join(dense_folder, 'cams')
    renamed_dir = os.path.join(dense_folder, 'images')
    ...
    for i in range(num_images):
        with open(os.path.join(cam_dir, '%08d_cam.txt' % i), 'w') as f:
            ...
            for j in range(4):
                for k in range(4):
                    f.write("{:.6f}".format(extrinsic[i+1][j, k]) + ' ')
                f.write('\n')
            f.write('\nintrinsic\n')
            for j in range(3):
                for k in range(3):
